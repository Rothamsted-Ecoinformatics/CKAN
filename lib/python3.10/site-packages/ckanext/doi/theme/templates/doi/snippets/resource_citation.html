...
{% block citation %}
    <section class="module additional-info">
        <div class="module-content citation">
            ...
            document.getElementById("resourceCitationDiv").innerHTML = dispVal + " ({{ h.package_get_year(pkg_dict) }}). <i>{{ res['name'] }}</i> (from <i>{{ pkg_dict['title'] }}, doi: https://doi.org/{{ pkg_dict['doi'] }}</i>) [Data set resource]. {{ pkg_dict['doi_publisher'] }}.";
            </script>
