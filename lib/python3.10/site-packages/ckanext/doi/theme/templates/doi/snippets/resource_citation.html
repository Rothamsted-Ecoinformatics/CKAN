    ...
    <section class="module additional-info">
        <div class="module-content citation">
            ...
            <div id="citation-string" class="citation-string">
                ...
                {{ pkg_dict['author'] }} ({{ h.package_get_year(pkg_dict) }}). <i>{{ res['name'] }}</i> (from <i>{{ pkg_dict['title'] }}</i>, doi: <i>https://doi.org/{{ pkg_dict['doi'] }}</i>) [Data set resource]. {{ pkg_dict['doi_publisher'] }}.
                {% block citation_link %}
