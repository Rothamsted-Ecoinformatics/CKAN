...
<section class="additional-info">
    ...
    <div class="citation">
        ...
        <script>
            ...
            let dispVal = "";
            if(citationName.length>1){
            dispVal =  citationName.slice(0, -1).join(", ") + ' and ' + citationName[citationName.length - 1];;
            }
            else{
                dispVal = citationName[0];
            }    
            document.getElementById("citationDiv").innerHTML = dispVal + " ({{ h.package_get_year(pkg_dict) }}). <i>{{ pkg_dict['title'] }}</i> [Data set]. {{ pkg_dict['doi_publisher'] }}.";
        </script>
        {% block citation_status %}
        <div id="citation-status">
            {% if not pkg_dict['doi_status'] %}
            <div>
                <span class="dataset-private label label-inverse" title="This DOI has not yet been published">
            ...
            <div>
                ...
                <a class="btn btn-mini" title="Download BibTeX"
                    href="https://{{ datacite_url }}/dois/application/x-bibtex/{{ pkg_dict['doi'] }}">
